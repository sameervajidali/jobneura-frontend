...
export default function TicketDetailsPage() {
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      ...
      setTicket(updated.ticket || updated);
    } catch (err) {
      console.error(err);
    } finally {
      setSaving(false);
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="p-6">
          <h1 className="text-2xl font-semibold text-gray-800 mb-2">{ticket.subject}</h1>
          <p className="text-sm text-gray-500 mb-4">
            By <strong>{ticket.user.name}</strong> ({ticket.user.email}) • {new Date(ticket.createdAt).toLocaleString()}
          </p>
                ...
                {ticket.comments.map((c) => (
                  <li key={c._id} className="bg-gray-50 p-4 rounded border">
                    <p className="text-sm text-gray-600 mb-1">
                      <strong>{c.by.name}</strong> • {new Date(c.createdAt).toLocaleString()}
                    </p>
