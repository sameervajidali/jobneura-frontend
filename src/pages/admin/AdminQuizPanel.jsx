...
export default function AdminQuizPanel() {
  ...
  const filtered = quizzes.filter((q) => {
    const term = searchTerm.toLowerCase();
    const topicName =
      q.topic && typeof q.topic === 'object' && q.topic.name
        ? q.topic.name
        : typeof q.topic === 'string'
        ? q.topic
        : '';
    const categoryName =
      q.category && typeof q.category === 'object' && q.category.name
        ? q.category.name
        : typeof q.category === 'string'
        ? q.category
        : '';
    const subTopicName =
      q.subTopic && typeof q.subTopic === 'object' && q.subTopic.name
        ? q.subTopic.name
        : typeof q.subTopic === 'string'
        ? q.subTopic
        : '';
      ...
      <div className="overflow-x-auto bg-white rounded shadow">
        <table className="min-w-full divide-y divide-gray-200 table-auto">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? (
              <tr>
                <td colSpan={13} className="py-8 text-center text-gray-500">Loading…</td>
              </tr>
            ) : error ? (
              <tr>
                <td colSpan={13} className="py-8 text-center text-red-500">{error}</td>
              </tr>
            ) : paginated.length === 0 ? (
              <tr>
                <td colSpan={13} className="py-8 text-center text-gray-500">No quizzes found.</td>
              </tr>
            ) : (
              paginated.map((q, idx) => (
                <tr key={q._id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-6 py-2 text-sm text-indigo-600 text-center">
                    <Link to={`/admin/quizzes/${q._id}/questions`} className="hover:underline">
                      {typeof q.topic === 'object' && q.topic && q.topic.name
                        ? q.topic.name
                        : typeof q.topic === 'string'
                        ? q.topic
                        : ''}
                  ...
                  <td className="px-6 py-2 text-sm text-gray-700 text-center">
                    {typeof q.category === 'object' && q.category && q.category.name
                      ? q.category.name
                      : typeof q.category === 'string'
                      ? q.category
                      : ''}
