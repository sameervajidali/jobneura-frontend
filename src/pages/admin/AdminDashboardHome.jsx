...
// src/pages/admin/AdminDashboardHome.jsx
import React, { useState, useEffect } from "react";
import { FaUsers, FaBook, FaChartLine, FaTicketAlt } from "react-icons/fa";
import {
  LineChart,
  Line,
  XAxis,
...
export default function AdminDashboardHome() {
  const [loading, setLoading] = useState(true);
  const [stats, setStats] = useState({ totalUsers: 0, totalQuizzes: 0, leaderboardEntries: 0, openTickets: 0 });
  const [dau, setDau] = useState([]);
  const [categories, setCategories] = useState([]);
  const [recentUsers, setRecentUsers] = useState([]);
  const [recentQuizzes, setRecentQuizzes] = useState([]);
  ...
  useEffect(() => {
    async function fetchAll() {
      setLoading(true);
      try {
        const [usersPage, allQuizzes, leaderboardPage, ticketsRes] = await Promise.all([
          API.get("/admin/users?limit=1&page=1"),
          API.get("/quizzes/admin/quizzes"),
          API.get("/quizzes/leaderboard", { params: { page: 1, limit: 1 } }),
          API.get("/ticket/admin/tickets?status=open").catch(() => ({ data: { total: 0, tickets: [] } })),
        ...
        setStats({
          totalUsers: usersPage.data.total || 0,
          totalQuizzes: Array.isArray(allQuizzes.data) ? allQuizzes.data.length : 0,
          leaderboardEntries: leaderboardPage.data.total || 0,
          openTickets: ticketsRes.data.total || 0,
        });
        setTickets(ticketsRes.data.tickets || []);
        ...
        const { data: recentUsersPage } = await API.get("/admin/users?limit=5&page=1");
        setRecentUsers(Array.isArray(recentUsersPage.data.users) ? recentUsersPage.data.users : []);

        const { data: recentQuizzesList } = await API.get("/quizzes/admin/quizzes?limit=5&page=1");
        setRecentQuizzes(Array.isArray(recentQuizzesList.data) ? recentQuizzesList.data : []);
      } catch (err) {
        console.error("Dashboard fetch error:", err);
      } finally {
        setLoading(false);
  ...
  return (
    <div className="space-y-8 p-6">
      ...
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        {statCards.map(({ label, value, icon: Icon, bg, color }) => (
          <div key={label} className="bg-white p-6 rounded-lg shadow h-32 flex items-center">
            <div className={`${bg} p-3 rounded-full mr-4`}><Icon className={`${color} text-2xl`} /></div>
            <div>
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
        {/* Daily Active Users */}
        <div className="bg-white p-6 rounded-lg shadow h-80 flex flex-col">
          <h4 className="text-lg font-semibold mb-4">Daily Active Users (Last 7 Days)</h4>
          {dau.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={dau} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="date" tick={{ fontSize: 12 }} />
                <YAxis tick={{ fontSize: 12 }} />
                <Tooltip />
                <Line type="monotone" dataKey="count" stroke={LINE_COLOR} strokeWidth={2} dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          ) : (
          ...
          <div className="overflow-auto max-h-48">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {recentQuizzes.map((q) => (
                  <tr key={q._id}>
                    <td className="py-2 text-gray-700 break-words">{q.title}</td>
                    <td className="py-2 text-gray-700">{q.questions?.length ?? "–"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {tickets.length > 0 ? (
                  tickets.map((t) => (
                    <tr key={t._id}>
                      <td className="py-2 text-gray-700 break-words">{t.subject}</td>
                      <td className="py-2 text-gray-700">{t.status}</td>
                    </tr>
                  ))
                ) : (
