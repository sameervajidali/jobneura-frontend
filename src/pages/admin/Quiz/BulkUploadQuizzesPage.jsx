...
// src/pages/admin/Quiz/BulkUploadQuestionsPage.jsx
import React, { useState } from 'react';
import { bulkUploadQuestionsFile } from '../../../services/quizService';

export default function BulkUploadQuestions({ quizId }) {
  const [file, setFile] = useState(null);
  ...
  const handleUpload = async () => {
    if (!file) return setStatus('Please select a CSV/XLSX file first.');
    setStatus('Uploading…');
    try {
      await bulkUploadQuestionsFile(quizId, file);
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-600">Upload a CSV or XLSX of questions to add them all at once.</p>
