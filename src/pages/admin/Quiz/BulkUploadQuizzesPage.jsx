...
export default function BulkUploadQuizzesPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      // bulkUploadQuizzes returns the CSV blob directly
      const blob = await bulkUploadQuizzes(file, { responseType: 'blob' });
      saveAs(blob, 'quizzes_upload_report.csv');
      setStatus('✅ Upload successful, report downloaded!');
    } catch (err) {
      console.error('Bulk quizzes upload error:', err);
      const msg = err.response?.data?.message || err.message;
      setStatus(`❌ Upload failed: ${msg}`);
  ...
  return (
    <div className="p-6 max-w-lg mx-auto bg-white rounded shadow">
      <h2 className="text-2xl font-semibold mb-4">Bulk Upload Quizzes</h2>
      <p className="text-gray-600 mb-3">
        Upload a CSV file containing quizzes (title, category, topic, level, duration, totalMarks, isActive).<br />
        After upload, a CSV report will be automatically downloaded.
      </p>
      <input
        type="file"
        accept=".csv"
