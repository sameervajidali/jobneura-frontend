...
import { Badge, Button } from '@/components/ui'; // adjust to your UI library
...
export default function QuizPage() {
  ...
  const displayed = useMemo(() => {
    ...
    if (filters.categories.length) {
      list = list.filter(q => filters.categories.includes(q.category));
    }
  ...
  return (
    <div className="flex gap-6">
      ...
      <aside className="hidden lg:block w-60 bg-white dark:bg-gray-800 p-4 rounded-xl shadow">
        <h3 className="font-semibold mb-3 text-gray-700 dark:text-gray-200">Filters</h3>
        <div className="space-y-4">
          ...
          <div>
            <h4 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Difficulty</h4>
      ...
      <div className="flex-1 space-y-6">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-gray-800 dark:text-gray-100">Available Quizzes</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">Sharpen your skills with bite-sized tests</p>
          </div>
        ...
        >
          {displayed.map(q => (
            ...
            >
              <div>
                <h3 className="text-lg font-medium text-gray-800 dark:text-gray-100 mb-1">{q.title}</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-2 truncate">{q.description}</p>
              </div>
              <div className="flex items-center justify-between mt-4 text-sm">
                <Badge variant="outline">{q.category}</Badge>
                <Badge variant="solid">{q.difficulty}</Badge>
              </div>
        ...
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-white dark:bg-gray-800 p-4 rounded-full shadow-lg flex items-center gap-4">
          ...
          <Button
            onClick={() => navigate('/dashboard/quizzes/resume')}
          >
