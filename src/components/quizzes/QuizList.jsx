...
export default function useQuizList(filters = {}) {
  ...
  useEffect(() => {
    ...
    const fetchQuizzes = async () => {
      ...
      try {
        ...
        setState(prev => ({
          ...prev,
          quizzes: Array.isArray(data.items) ? data.items : [],
          total: data.total || 0,
          page: data.page || 1,
          limit: data.limit || 10,
          loading: false,
        ...
        setState(prev => ({
          ...prev,
          error: err?.response?.data?.message || err.message || 'Failed to load quizzes',
          loading: false,
        }));
      }
    };
  ...
  }, [JSON.stringify(filters)]); // or more granular dependency array
