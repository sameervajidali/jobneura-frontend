...
export default function QuizList({ filters = {}, onPageChange }) {
  ...
  return (
    <>
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 transition-all">
        {quizzes.map((q) => {
          const isNew =
            q.createdAt &&
          ...
          return (
            <div
              key={q._id}
              className="flex flex-col bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm hover:shadow-lg p-6 transition-all duration-200 hover:-translate-y-1 min-h-[235px] relative"
            >
              {/* Category + New */}
              ...
              <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100 mb-4 flex-1 leading-tight line-clamp-2">
                {q.subTopic?.name || q.title || "Untitled Quiz"}
              </h3>
      ...
      {total > limit && (
        <div className="mt-10 flex justify-center items-center space-x-4">
          <button
            ...
            disabled={page === 1}
            className="px-4 py-2 bg-gray-200 dark:bg-gray-800 rounded-full hover:bg-indigo-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-100 font-medium disabled:opacity-50 disabled:cursor-not-allowed transition"
          >
            Prev
          </button>
