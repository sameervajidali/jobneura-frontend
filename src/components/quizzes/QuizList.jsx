    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {quizzes.map((q) => (
          <div
            key={q._id}
            className="relative flex flex-col bg-white rounded-lg shadow-sm hover:shadow-md transition font-sans min-h-[20rem]"
          >
            {/* New badge absolute */}
            {q.createdAt &&
              Date.now() - new Date(q.createdAt).getTime() <
                7 * 24 * 60 * 60 * 1000 && (
                <SparklesIcon
                  className="absolute top-4 right-4 w-6 h-6 text-green-400"
                  aria-label="New"
                />
              )}
            ...
            {/* Micro-actions */}
            <div className="px-6 flex justify-end space-x-4 text-gray-400">
              <BookmarkIcon
