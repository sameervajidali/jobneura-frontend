...
// AppInitializer: performs one-time redirect after session loads
function AppInitializer({ children }) {
  const { user, loading } = useAuth();
  const location = useLocation();
  const navigate = useNavigate();
  ...
  useEffect(() => {
    if (!loading && !didRedirect.current) {
      ...
      if (!user) return; // no user: stay on public routes
...
// LayoutWrapper: renders Navbar/Footer and App routes
function LayoutWrapper() {
  const { user } = useAuth();
  const location = useLocation();
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className={`flex-grow ${!isDashboard && !isAdmin ? 'pt-16' : ''}`}>        
        <Routes>
          ...
          {/* User Dashboard */}
          <Route path="/dashboard" element={<ProtectedRoute allowedRoles={[] /* all roles */} />}>
          ...
          <Route path="/admin/*" element={<ProtectedRoute allowedRoles={ADMIN_ROLES} />}>
            <Route path="" element={<AdminDashboardLayout><AdminDashboardHome /></AdminDashboardLayout>} />
            <Route path="users" element={<AdminDashboardLayout><AdminUsersPage /></AdminDashboardLayout>} />
            {/* ...other admin routes similarly wrapped in AdminDashboardLayout... */}
          </Route>
